...
if (!empty($pagina)) {
    ...
    $query_usuarios = "SELECT id, setor, responsavel, title, start, end, local, tsinal  FROM events ORDER BY start DESC LIMIT $inicio, $qnt_result_pg";
    ...
    while ($row_usuario = $result_usuarios->fetch(PDO::FETCH_ASSOC)) {
        extract($row_usuario);
          ...
          $inicio = date("d/m/Y H:i",strtotime($row_usuario ["start"]));
                    $fim = date("d/m/Y H:i",strtotime($row_usuario ["end"]));
        ...
        $dados .= "<tr>
                    ...
                    <td>
                        ...
                        <button id='$id' class='btn btn-outline-danger btn-sm' onclick='apagarUsuarioDados($id)'>x</button>
                    </td>
                </tr>";
    }
    ...
    $query_pg = "SELECT COUNT(id) AS num_result FROM events";
