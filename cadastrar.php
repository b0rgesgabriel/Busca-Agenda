...
if (empty($dados['nome'])) {
    ...
    $query_usuario = "INSERT INTO usuarios (nome, email, atendimento, local) VALUES (:nome, :email, :atendimento, :local)";
    $cad_usuario = $conn->prepare($query_usuario);
    $cad_usuario->bindParam(':nome', $dados['nome']);
    $cad_usuario->bindParam(':email', $dados['email']);
     $cad_usuario->bindParam(':atendimento', $dados['atendimento']);
     $cad_usuario->bindParam(':local', $dados['local']);
    $cad_usuario->execute();
